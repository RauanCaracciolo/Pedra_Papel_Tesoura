...
class WarActivity: AppCompatActivity() {
    ...
    private fun battle(){
        ...
        if(weaponPlayer1 != null && weaponPlayer2 != null){
            ...
            if(winner != null){
                Toast.makeText(
                    this, "${getString(R.string.winner)} ${winner.name}", Toast.LENGTH_LONG)
                    .show()
            }else{
                Toast.makeText(this, getString(R.string.draw), Toast.LENGTH_LONG)
                    .show()
            }
            weaponPlayer1 = null
            weaponPlayer2 = null
            ...
            Toast.makeText(this, "$name${getString(R.string.chhose_gum_player)}", Toast.LENGTH_LONG)
                .show()
        }
    }
            ...
            result ->
                if(result.resultCode == Activity.RESULT_OK){
                        ...
                        if(extras != null){
                            ...
                            val choseWeapon: Weapon = extras.getSerializable(Constants.KEY_WEAPON, Weapon::class.java) as Weapon
                            if(number == 1){
                                weaponPlayer1 = choseWeapon
                            }
                            if(number == 2){
                                weaponPlayer2 = choseWeapon
                            }
                        }
                }
        }
    ...
    private fun startSelectionActivity(number: Int){
        ...
        val mIntend = Intent(this, SelectionActivity::class.java)

        mIntend.putExtra(Constants.KEY_PLAYER_NUMBER, number)
        mIntend.putExtra(Constants.KEY_PLAY_NAME, name)

        resultLauncher.launch(mIntend)
    }
